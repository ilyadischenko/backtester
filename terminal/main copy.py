...
from logic.heatmap import HeatmapRenderer, build_heatmap
...
class DataLoader(QThread):
    ...
    def run(self):
        try:
            ...
            trades_df = data_manager.load_timerange(
            ...
            self.progress.emit("Загрузка стакана...")
            depth_df = data_manager.load_timerange(
                exchange="binance", symbol=self.symbol,
                start_time=self.start_time, end_time=self.end_time,
                data_type="depth")
            ...
            self.finished.emit({"trades": trades_cache, "depth": depth_cache})
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.start_dt = QDateTimeEdit(
            QDateTime.fromString("2025-06-25 13:00:00", "yyyy-MM-dd HH:mm:ss"))
        self.end_dt = QDateTimeEdit(
            QDateTime.fromString("2025-06-25 13:05:00", "yyyy-MM-dd HH:mm:ss"))
    ...
    def _toggle_heatmap(self, checked):
        self._heatmap.set_visible(checked)
